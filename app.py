    ...
    **Optional:**
    ...
    if st.button("🚀 Process Files", type="primary", use_container_width=True):
        if not files.get("links") or not files.get("embeddings"):
            render_warning_box("⚠️ Please upload at least Internal Links and Embeddings files from Screaming Frog.")
        else:
        ...
        with col3:
            ...
            if st.session_state.use_dataforseo and dataforseo_client is not None and st.session_state.url_keywords_map:
                ...
                if len(flat_keywords) > 0:
                    ...
                    if st.button(f"📊 Get Search Volume ({len(flat_keywords)} kw, {batch_count} batch{'es' if batch_count > 1 else ''})", use_container_width=True):
